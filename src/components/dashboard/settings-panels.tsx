    ...
    <div className="space-y-6">
      <Card className="glass border-0">
        <CardContent className="space-y-5 pt-6">
          <SectionHeader title="Core" description="Prefix, currency and where moderation is logged." />
          <div className="grid gap-5 md:grid-cols-2">
            <Field label="Legacy prefix" hint="Slash commands always work; this is for text commands.">
              <Input value={draft.prefix} maxLength={5} onChange={(e) => set("prefix", e.target.value)} />
            </Field>
      ...
      <Card className="glass border-0">
        <CardContent className="space-y-5 pt-6">
          <SectionHeader title="XP & levels" description="Reward active crew members for chatting." />
          <ToggleRow
      ...
      <Card className="glass border-0">
        <CardContent className="space-y-5 pt-6">
          <SectionHeader title="Tickets" description="Support requests handled in private channels." />
          <ToggleRow
        ...
        <CardContent className="space-y-5 pt-6">
          ...
          {draft.dynamic_image_enabled && (
            <div className="rounded-xl border border-dashed border-border/70 p-4 text-center text-sm text-muted-foreground">
              ...
              try{" "}
              <code className="rounded bg-secondary/50 px-1.5 py-0.5">/profile</code> style
...
export function LoggingPanel({ guildId, config, onSaved }: PanelProps) {
  ...
  return (
    <div className="space-y-6">
      <Card className="glass border-0">
        <CardContent className="space-y-5 pt-6">
          <SectionHeader title="Event logging" description="Keep a written record of everything on deck." />
          <ToggleRow
            label="Enable logging"
            checked={draft.enabled}
            onChange={(v) => set("enabled", v)}
          />
          <Field label="Default log channel" hint="Used by the original broad category toggles below.">
            <PickerSelect
            ...
            <div key={category.title} className="space-y-2 rounded-xl border border-border/70 p-4">
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {category.events.map((evt) => (
                  <div key={evt.key} className="flex items-center justify-between gap-3 rounded-lg bg-secondary/30 px-3 py-2">
