...
async function sessionOrNull() {
  ...
  return sessionFromHeader(cookie());
}

async function requireAdmin() {
  const { requireAdmin: authorizeAdmin } = await import("@/lib/admin.server");
  const auth = await authorizeAdmin(cookie());
  const session = await sessionOrNull();
  if (!session) throw new Error("Please sign in with Discord.");
  return { session, supabaseAdmin: auth.supabaseAdmin };
}
...
export const listPlanUnlockRequests = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data, error } = await supabaseAdmin.from("plan_unlock_requests").select("id,requester_user_id,requester_username,guild_id,plan_id,unlock_method,status,task_progress,review_note,reviewed_by,requested_at,reviewed_at,plans(name,key,features)").order("requested_at", { ascending: false }).limit(200);
  if (error) throw new Error(`Could not load Premium requests: ${error.message}`);
  return data ?? [];
});
...
export const reviewPlanUnlockRequest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { session, supabaseAdmin } = await requireAdmin();
    const { data: request, error: requestError } = await supabaseAdmin.from("plan_unlock_requests").select("id,requester_user_id,plan_id,status,plans(key,name,features)").eq("id", data.requestId).maybeSingle();
    if (requestError) throw new Error(`Could not load unlock request: ${requestError.message}`);
    ...
    if (data.decision === "approved") {
      ...
      if (userError) throw new Error(`Could not unlock the requested plan: ${userError.message}`);
    }
    const { error: updateError } = await supabaseAdmin.from("plan_unlock_requests").update({ status: data.decision, review_note: data.note?.trim() || null, reviewed_by: session.userId, reviewed_at: new Date().toISOString() }).eq("id", data.requestId);
...
const taskInput = z.object({ id: uuid.optional(), planId: uuid, title: z.string().min(2).max(120), description: z.string().max(500).nullable().optional(), taskType: z.enum(["message_count", "messages", "voice_minutes", "reaction_count", "reactions", "member_count"]), targetValue: z.number().int().min(1).max(1000000000), sortOrder: z.number().int().min(0).max(10000).default(0) });
...
export const savePlanTask = createServerFn({ method: "POST" }).inputValidator((data: unknown) => taskInput.parse(data)).handler(async ({ data }) => {
  const { supabaseAdmin } = await requireAdmin();
...
export const deletePlanTask = createServerFn({ method: "POST" }).inputValidator((data: unknown) => z.object({ id: uuid }).parse(data)).handler(async ({ data }) => {
  const { supabaseAdmin } = await requireAdmin();
