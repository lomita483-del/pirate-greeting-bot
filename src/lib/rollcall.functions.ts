...
export const getRollCallDashboard = createServerFn({ method: "GET" }).inputValidator((data: unknown) => guildInput.parse(data)).handler(async ({ data }) => {
  ...
  if (settings.error) throw new Error(settings.error.message); if (calls.error) throw new Error(calls.error.message);
  const rows = calls.data ?? []; const ids = rows.map((r) => r.id);
...
export const saveRollCallSettings = createServerFn({ method: "POST" }).inputValidator((data: unknown) => settingsInput.parse(data)).handler(async ({ data }) => {
  ...
  const { error } = await supabaseAdmin.from("roll_call_settings").upsert(payload, { onConflict: "guild_id" }); if (error) throw new Error(error.message);
  ...
  await supabaseAdmin.from("dashboard_access_log").insert({ discord_user_id: session.userId, discord_username: session.username, guild_id: data.guildId, action: "update:rollcall" }); return { ok: true };
});
