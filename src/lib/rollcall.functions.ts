  ...
  .handler(async ({ data }) => {
    ...
    const { data: responses, error: responseError } = ids.length
      ? await supabaseAdmin
          ...
          .select("id, roll_call_id, user_id, username, display_name, responded_at")
          .in("roll_call_id", ids)
...
const settingsInput = z.object({
  ...
  dailyDurationHours: z.number().int().min(1).max(336),
});
...
export const saveRollCallSettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const payload = {
      ...
      manager_role_ids: data.managerRoleIds,
      default_channel_id: data.defaultChannelId,
      daily_enabled: data.dailyEnabled,
      daily_hour_utc: data.dailyHourUtc,
      daily_target_role_ids: data.dailyTargetRoleIds,
      daily_title: data.dailyTitle,
      daily_description: data.dailyDescription,
      daily_duration_hours: data.dailyDurationHours,
...
const startInput = z.object({
  ...
  durationHours: z.number().min(0.25).max(336),
  channelId: snowflake,
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rollCall, error } = await supabaseAdmin
      ...
      .insert({
        ...
        target_role_ids: data.targetRoleIds,
    ...
    const { error: queueError } = await supabaseAdmin.from("bot_action_queue").insert({
      guild_id: data.guildId,
      action: "rollcall_start",
      target_id: String(rollCall.id),
      payload: { roll_call_id: rollCall.id },
      requested_by: session.userId,
      requested_by_name: session.username,
      status: "pending",
    });
    if (queueError) {
...
export const closeRollCall = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await supabaseAdmin.from("bot_action_queue").insert({
      guild_id: data.guildId,
      action: "rollcall_close",
      target_id: data.id,
      payload: { roll_call_id: data.id },
      requested_by: session.userId,
      requested_by_name: session.username,
      status: "pending",
    });
