...
export const ALL_COMMANDS: (CommandEntry & { category: string; categoryTitle: string })[] =
  COMMAND_CATEGORIES.flatMap((c) =>
    c.commands.map((e) => ({ ...e, category: c.slug, categoryTitle: c.title })),
  );

/** Slash path a member actually types in Discord. */
export function commandPath(category: string, entry: CommandEntry): string {
