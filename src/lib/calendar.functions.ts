  ...
  .handler(async ({ data }) => {
    ...
    const notifiers = (notifiersRes.data ?? []).map((row) => {
      ...
      return {
        ...
        // "keep" | "delete_previous" | "edit_previous" — matches the DB
        // column exactly; this is the only cleanup setting a notifier has.
        cleanupMode: (r["cleanup_mode"] as string) ?? "keep",
...
export const saveNotifier = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) =>
    z
      .object({
        ...
        // "keep" | "delete_previous" | "edit_previous" — this is the ONLY
        // cleanup setting; there is no separate "enabled" column in the DB.
        cleanupMode: z.enum(["keep", "delete_previous", "edit_previous"]).optional(),
  ...
  .handler(async ({ data }) => {
    ...
    const payload = {
      ...
      cleanup_mode: data.cleanupMode ?? "keep",
