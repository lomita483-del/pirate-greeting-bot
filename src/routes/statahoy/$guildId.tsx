      ...
      <main className="mx-auto max-w-7xl px-6 pb-24">
        ...
        {data && (
          <>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <RankTable title="Top message senders" rows={data.topUsers} format={(v) => v.toLocaleString()} label={(r) => r.name ?? r.id} />
              <RankTable title="Top voice members" rows={data.topVoiceUsers} format={humanizeSeconds} label={(r) => r.name ?? r.id} />
              <RankTable title="Top channels" rows={data.topChannels} format={(v) => v.toLocaleString()} label={(r) => `#${r.id}`} />
            </div>
          </>
        )}
      </main>
