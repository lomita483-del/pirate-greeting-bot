...
function Overview({ context, onJump }: { context: AdminContext; onJump: (tab: TabKey) => void }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="relative mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8"><QuickAction icon={Users} label="Users" onClick={() => onJump("users")} /><QuickAction icon={Server} label="Servers" onClick={() => onJump("servers")} /><QuickAction icon={Bell} label="Send Notify" onClick={() => onJump("notifications")} /><QuickAction icon={ListChecks} label="Premium Tasks" onClick={() => onJump("tasks")} /><QuickAction icon={AlertTriangle} label="ERROR & FEEDBACK" onClick={() => onJump("errors")} /><QuickAction icon={UserRound} label="Users Reports" onClick={() => onJump("reports")} /><button type="button" onClick={() => window.location.assign("/owner-console/plan-requests")} className="group flex items-center justify-center gap-2 rounded-xl border border-primary/20 bg-primary/[0.055] px-3 py-3 text-xs font-semibold text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Crown className="size-4" />Premium Requests</button><QuickAction icon={Settings} label="Staff" onClick={() => onJump("staff")} /></div></section>
