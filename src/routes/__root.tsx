...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=1280, initial-scale=0.25, maximum-scale=1, user-scalable=no" },
      { name: "author", content: "!HOY BOT" },
      ...
      { name: "apple-mobile-web-app-title", content: "!HOY BOT" },
    ],
    links: [
      { rel: "stylesheet", href: appCss },
      { rel: "preconnect", href: "https://fonts.googleapis.com" },
      { rel: "preconnect", href: "https://fonts.gstatic.com", crossOrigin: "anonymous" },
      { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Sora:wght@500;600;700&family=Manrope:wght@400;500;600&display=swap" },
      { rel: "icon", href: "/hoy-logo.svg", type: "image/svg+xml" },
      { rel: "apple-touch-icon", href: "/hoy-logo.svg" },
...
function RootComponent() {
  ...
  useEffect(() => { const orientation = typeof screen !== "undefined" ? (screen.orientation as ScreenOrientation & { lock?: (o: string) => Promise<void> } | undefined) : undefined; orientation?.lock?.("portrait").catch(() => {}); }, []);
  useEffect(() => { if (typeof window === "undefined" || !("serviceWorker" in navigator)) return; navigator.serviceWorker.register("/sw.js").catch((error) => { console.error("Service worker registration failed", error); }); }, []);
  return <QueryClientProvider client={queryClient}><div aria-hidden className="ahoy-watermark" /><div className="relative z-10"><Outlet /></div><Toaster position="top-right" richColors /></QueryClientProvider>;
}
