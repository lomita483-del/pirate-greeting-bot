...
class XPEvents(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        try:
            ...
            if new_level is None: return
            try: await self.bot.levels.apply_rewards(message.author, new_level)  # type: ignore[attr-defined]
            except Exception as exc: log.warning("Level reward processing failed in %s: %s", message.guild.id, exc)
            ...
            if not channel_id or not settings.get("level_up_card_enabled", True): return
            ...
            card.add_field(name="Total XP", value=f"**{xp:,}**", inline=True)
        ...
        except Exception as exc: log.warning("XP processing failed in %s: %s", message.guild.id, exc)

async def setup(bot: commands.Bot) -> None: await bot.add_cog(XPEvents(bot))
