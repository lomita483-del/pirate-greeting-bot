"""Statahoy tracking: per-day counters plus per-user activity analytics."""
...
class StatsEvents(commands.Cog):
    ...
    async def _touch_user(
        ...
        *,
            ...
            payload.update(
                {
                    ...
                    "last_seen_at": now,
                    "message_count": int(current.get("message_count", 0) or 0) + 1,
                }
            )
        ...
        if status is not None:
            payload.update({"last_seen_at": now, "last_online_status": status})
        ...
        if voice_join_at:
            payload.update(
                {
                    "last_voice_join_at": voice_join_at,
                    "last_seen_at": now,
                    "voice_session_count": int(current.get("voice_session_count", 0) or 0) + 1,
                }
            )

        if voice_leave_at:
            session_seconds = _seconds_between(current.get("last_voice_join_at"), voice_leave_at)
            payload.update(
                {
                    "last_voice_leave_at": voice_leave_at,
                    "last_seen_at": now,
                    "voice_seconds": int(current.get("voice_seconds", 0) or 0) + session_seconds,
                }
            )

        for key in (
            "username", "last_seen_at", "last_online_at", "last_online_status",
            "last_message_at", "last_message_content", "last_message_channel_id",
            "last_command_at", "last_command_name", "command_count",
            "message_count", "last_voice_join_at", "last_voice_leave_at",
            "voice_seconds", "voice_session_count",
        ):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        try:
            await repo.bump_message_activity(
                ...
                str(message.channel.id), _today(),
            )
            await repo.increment_server_counter(str(message.guild.id), "message_count_total", 1)
        except Exception as exc:
    ...
    async def activity_command(
        ...
        if member is not None:
            ...
            message_total = int(row.get("message_count", 0) or 0)
            await interaction.followup.send(
                embed=self._activity_embed(member, row, voice, message_total), ephemeral=True
            )
            return

        try:
            result = await repo.db.try_run(
                lambda c: c.table("user_activity").select("*")
                .eq("guild_id", str(guild.id)).order("last_seen_at", desc=True).limit(10).execute()
            )
            rows = getattr(result, "data", None) or []
        except Exception as exc:
        ...
        if not rows:
            embed.description = "No user activity has been tracked yet."
        else:
            for row in rows:
                ...
                seen = discord.utils.parse_time(row.get("last_seen_at")) if row.get("last_seen_at") else None
                embed.add_field(
                    name=f"{name} · {status}",
                    value=(f"Messages: **{int(row.get('message_count', 0) or 0):,}** · "
                           f"Bot uses: **{int(row.get('command_count', 0) or 0):,}**\n"
                           f"Last seen: {discord.utils.format_dt(seen, 'R') if seen else 'Never'}"),
                    inline=False,
                )
    ...
    @staticmethod
    def _activity_embed(member: discord.Member, row: dict[str, Any], voice: dict[str, Any], message_total: int) -> discord.Embed:
        ...
        last_online = discord.utils.parse_time(row.get("last_online_at")) if row.get("last_online_at") else None
        ...
        last_voice_leave = discord.utils.parse_time(row.get("last_voice_leave_at")) if row.get("last_voice_leave_at") else None
        voice_seconds = int(row.get("voice_seconds", voice.get("voice_seconds", 0)) or 0)
        sessions = int(row.get("voice_session_count", voice.get("sessions", 0)) or 0)

        embed = embeds.brand(f"{member.display_name} · Activity", f"Last known Discord status: **{status}**")
        ...
        embed.add_field(name="Voice time", value=humanize(voice_seconds))
        embed.add_field(name="Voice sessions", value=f"{sessions:,}")
        embed.add_field(name="Last seen", value=discord.utils.format_dt(last_seen, "R") if last_seen else "Never")
        embed.add_field(name="Last came online", value=discord.utils.format_dt(last_online, "R") if last_online else "Not recorded")
        ...
        embed.add_field(name="Last voice leave", value=discord.utils.format_dt(last_voice_leave, "R") if last_voice_leave else "Never")
        embed.add_field(name="Last message sent", value=_clip(str(row.get("last_message_content") or "No text message recorded."), 900), inline=False)
        embed.set_footer(text="Statahoy ⚓")
        return embed
