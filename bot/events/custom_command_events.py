"""Execute dashboard custom commands and load the centralized error recorder."""
...
class CustomCommandEvents(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        try: rows = await repo.custom_commands(str(message.guild.id))
        except Exception: log.exception("Failed to load custom commands for guild %s", message.guild.id); return
        if not rows: return
        prefix = "!"
        try:
        ...
        if not body: return
        parts = body.split(maxsplit=1); name = parts[0].lower()
        ...
        try:
            if row.get("is_embed"):
                color = discord.Color.blurple(); raw_color = row.get("embed_color")
                if raw_color:
        ...
        except discord.HTTPException: log.exception("Custom command '%s' failed in guild %s", name, message.guild.id)
