...
class Repository:
    ...
    async def upsert_server(self, guild_id: str, name: str, icon: Optional[str], owner_id: Optional[str], member_count: int) -> None:
        payload = {"guild_id": guild_id, "name": name, "icon": icon, "owner_id": owner_id, "member_count": member_count, "bot_present": True}
        await self.db.try_run(lambda c: c.table("servers").upsert(payload, on_conflict="guild_id").execute())
        for table in ("server_settings", "welcome_settings", "logging_settings", "automod_settings", "role_settings"):
            await self.db.try_run(lambda c, t=table: c.table(t).upsert({"guild_id": guild_id}, on_conflict="guild_id", ignore_duplicates=True).execute())

    async def mark_server_left(self, guild_id: str) -> None:
        await self.db.try_run(lambda c: c.table("servers").update({"bot_present": False}).eq("guild_id", guild_id).execute())

    async def get_settings(self, guild_id: str, table: str = "server_settings") -> dict[str, Any]:
        rows = await self.db.try_run(lambda c: c.table(table).select("*").eq("guild_id", guild_id).limit(1).execute())
        data = getattr(rows, "data", None) or []
        return data[0] if data else {}

    async def upsert_member(self, guild_id: str, member: Any) -> None:
        payload = {"guild_id": guild_id, "user_id": str(member.id), "username": member.name, "display_name": member.display_name, "avatar": member.display_avatar.url if member.display_avatar else None, "is_bot": bool(member.bot), "joined_at": member.joined_at.isoformat() if getattr(member, "joined_at", None) else None, "left_at": None}
        await self.db.try_run(lambda c: c.table("members").upsert(payload, on_conflict="guild_id,user_id").execute())

    async def mark_member_left(self, guild_id: str, user_id: str) -> None:
        await self.db.try_run(lambda c: c.table("members").update({"left_at": _now()}).eq("guild_id", guild_id).eq("user_id", user_id).execute())
    ...
    async def add_warning(self, guild_id: str, user_id: str, username: str, moderator_id: str, moderator_name: str, reason: str) -> dict[str, Any]:
        result = await self.db.run(lambda c: c.table("warnings").insert({"guild_id": guild_id, "user_id": user_id, "username": username, "moderator_id": moderator_id, "moderator_name": moderator_name, "reason": reason}).execute())
        rows = getattr(result, "data", None) or [{}]
        return rows[0]

    async def list_warnings(self, guild_id: str, user_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("warnings").select("*").eq("guild_id", guild_id).eq("user_id", user_id).eq("active", True).order("created_at", desc=True).limit(25).execute())
        return getattr(rows, "data", None) or []

    async def log_action(self, guild_id: str, action: str, *, target_id: Optional[str] = None, target_name: Optional[str] = None, moderator_id: Optional[str] = None, moderator_name: Optional[str] = None, reason: Optional[str] = None, duration_seconds: Optional[int] = None, metadata: Optional[dict[str, Any]] = None) -> None:
        await self.db.try_run(lambda c: c.table("moderation_logs").insert({"guild_id": guild_id, "action": action, "target_id": target_id, "target_name": target_name, "moderator_id": moderator_id, "moderator_name": moderator_name, "reason": reason, "duration_seconds": duration_seconds, "metadata": metadata or {}}).execute())

    async def recent_actions(self, guild_id: str, limit: int = 10) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("moderation_logs").select("*").eq("guild_id", guild_id).order("created_at", desc=True).limit(limit).execute())
        return getattr(rows, "data", None) or []

    async def get_xp(self, guild_id: str, user_id: str) -> dict[str, Any]:
        rows = await self.db.try_run(lambda c: c.table("xp_profiles").select("*").eq("guild_id", guild_id).eq("user_id", user_id).limit(1).execute())
        data = getattr(rows, "data", None) or []
        return data[0] if data else {}

    async def save_xp(self, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("xp_profiles").upsert(payload, on_conflict="guild_id,user_id").execute())

    async def xp_leaderboard(self, guild_id: str, limit: int = 10) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("xp_profiles").select("*").eq("guild_id", guild_id).order("xp", desc=True).limit(limit).execute())
        return getattr(rows, "data", None) or []

    async def xp_rank(self, guild_id: str, xp: int) -> int:
        rows = await self.db.try_run(lambda c: c.table("xp_profiles").select("user_id").eq("guild_id", guild_id).gt("xp", xp).execute())
        return len(getattr(rows, "data", None) or []) + 1

    async def get_wallet(self, guild_id: str, user_id: str) -> dict[str, Any]:
        rows = await self.db.try_run(lambda c: c.table("economy_profiles").select("*").eq("guild_id", guild_id).eq("user_id", user_id).limit(1).execute())
        data = getattr(rows, "data", None) or []
        return data[0] if data else {}

    async def save_wallet(self, payload: dict[str, Any]) -> None:
        await self.db.run(lambda c: c.table("economy_profiles").upsert(payload, on_conflict="guild_id,user_id").execute())

    async def economy_leaderboard(self, guild_id: str, limit: int = 10) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("economy_profiles").select("*").eq("guild_id", guild_id).order("balance", desc=True).limit(limit).execute())
        return getattr(rows, "data", None) or []

    async def next_ticket_number(self, guild_id: str) -> int:
        rows = await self.db.try_run(lambda c: c.table("tickets").select("ticket_number").eq("guild_id", guild_id).order("ticket_number", desc=True).limit(1).execute())
        data = getattr(rows, "data", None) or []
    ...
    async def get_ticket_by_channel(self, channel_id: str) -> dict[str, Any]:
        rows = await self.db.try_run(lambda c: c.table("tickets").select("*").eq("channel_id", channel_id).limit(1).execute())
        data = getattr(rows, "data", None) or []
        return data[0] if data else {}

    async def update_ticket(self, ticket_id: str, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("tickets").update(payload).eq("id", ticket_id).execute())

    async def add_ticket_message(self, ticket_id: str, author: Any, content: str) -> None:
        await self.db.try_run(lambda c: c.table("ticket_messages").insert({"ticket_id": ticket_id, "author_id": str(author.id), "author_name": str(author), "content": content[:4000]}).execute())

    async def ticket_transcript(self, ticket_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("ticket_messages").select("*").eq("ticket_id", ticket_id).order("sent_at").limit(500).execute())
        return getattr(rows, "data", None) or []
    ...
    async def due_reminders(self) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("reminders").select("*").eq("delivered", False).lte("remind_at", _now()).limit(50).execute())
        return getattr(rows, "data", None) or []

    async def mark_reminder_delivered(self, reminder_id: str) -> None:
        await self.db.try_run(lambda c: c.table("reminders").update({"delivered": True}).eq("id", reminder_id).execute())
    ...
    async def due_event_reminders(self, limit: int = 25) -> list[dict[str, Any]]:
        ...
        events_rows = await self.db.try_run(lambda c: c.table("calendar_events").select("*").in_("id", event_ids).execute())
        events_by_id = {e["id"]: e for e in (getattr(events_rows, "data", None) or [])}
        for reminder in reminders: reminder["event"] = events_by_id.get(reminder.get("event_id"))
        return reminders
    ...
    async def mark_event_reminder_failed(self, reminder_id: str, error: str, attempts: int) -> None:
        ...
        await self.db.try_run(lambda c: c.table("event_reminders").update({"status": status, "attempts": attempts, "error": error[:500]}).eq("id", reminder_id).execute())

    async def calendar_event(self, event_id: str) -> Optional[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("calendar_events").select("*").eq("id", event_id).limit(1).execute())
        data = getattr(rows, "data", None) or []
    ...
    async def event_rsvp_counts(self, event_id: str) -> dict[str, int]:
        rows = await self.db.try_run(lambda c: c.table("event_rsvps").select("response").eq("event_id", event_id).limit(1000).execute())
    ...
    async def calendar_sources(self, guild_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("calendar_sources").select("*").eq("guild_id", guild_id).limit(50).execute())
        return getattr(rows, "data", None) or []

    async def upcoming_events(self, guild_id: str, limit: int = 10) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("calendar_events").select("*").eq("guild_id", guild_id).eq("status", "confirmed").gte("start_time", _now()).order("start_time").limit(limit).execute())
        return getattr(rows, "data", None) or []

    async def events_between(self, guild_id: str, start_iso: str, end_iso: str, limit: int = 25) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("calendar_events").select("*").eq("guild_id", guild_id).eq("status", "confirmed").gte("start_time", start_iso).lte("start_time", end_iso).order("start_time").limit(limit).execute())
        return getattr(rows, "data", None) or []

    async def event_notifiers(self, guild_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("event_notifiers").select("*").eq("guild_id", guild_id).limit(50).execute())
        return getattr(rows, "data", None) or []

    async def calendar_filters(self, guild_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("calendar_filters").select("*").eq("guild_id", guild_id).order("priority", desc=True).limit(50).execute())
        return getattr(rows, "data", None) or []

    async def calendar_job_log(self, guild_id: str, limit: int = 10) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("calendar_job_log").select("*").eq("guild_id", guild_id).order("created_at", desc=True).limit(limit).execute())
        return getattr(rows, "data", None) or []

    async def pending_reminder_count(self, guild_id: str) -> int:
        rows = await self.db.try_run(lambda c: c.table("event_reminders").select("id").eq("guild_id", guild_id).eq("status", "pending").limit(1000).execute())
    ...
    async def custom_commands(self, guild_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("custom_commands").select("*").eq("guild_id", guild_id).eq("enabled", True).limit(100).execute())
        return getattr(rows, "data", None) or []

    async def bump_custom_command(self, command_id: str, uses: int) -> None:
        await self.db.try_run(lambda c: c.table("custom_commands").update({"uses": uses + 1}).eq("id", command_id).execute())

    async def reaction_roles_for_message(self, message_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("reaction_roles").select("*").eq("message_id", message_id).limit(50).execute())
        return getattr(rows, "data", None) or []

    async def guild_reaction_roles(self, guild_id: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("reaction_roles").select("*").eq("guild_id", guild_id).order("created_at", desc=True).limit(200).execute())
        return getattr(rows, "data", None) or []

    async def remove_reaction_role(self, guild_id: str, message_id: str, emoji: str) -> None:
        await self.db.try_run(lambda c: c.table("reaction_roles").delete().eq("guild_id", guild_id).eq("message_id", message_id).eq("emoji", emoji).execute())
    ...
    async def update_giveaway(self, giveaway_id: str, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("giveaways").update(payload).eq("id", giveaway_id).execute())
    ...
    async def mark_notification(self, notification_id: str, status: str, error: Optional[str] = None) -> None:
        await self.db.try_run(lambda c: c.table("platform_notifications").update({"delivery_status": status, "delivery_error": error, "delivered_at": _now()}).eq("id", notification_id).execute())
    ...
    async def update_poll(self, poll_id: str, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("polls").update(payload).eq("id", poll_id).execute())
    ...
    async def save_starboard_entry(self, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("starboard_entries").upsert(payload, on_conflict="source_message_id").execute())
    ...
    async def update_announcement(self, announcement_id: str, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("scheduled_announcements").update(payload).eq("id", announcement_id).execute())
    ...
    async def mark_stat_channel(self, row_id: str, value: int) -> None:
        await self.db.try_run(lambda c: c.table("stat_channels").update({"last_value": value, "last_updated_at": _now()}).eq("id", row_id).execute())
    ...
    async def close_active_cases(self, guild_id: str, user_id: str, actions: list[str]) -> None:
        await self.db.try_run(lambda c: c.table("moderation_cases").update({"active": False}).eq("guild_id", guild_id).eq("target_id", user_id).eq("active", True).in_("action", actions).execute())
    ...
    async def update_case(self, case_id: str, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("moderation_cases").update(payload).eq("id", case_id).execute())
    ...
    async def user_cases(self, guild_id: str, user_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    async def log_activity(self, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("activity_logs").insert(payload).execute())
    ...
    async def finish_bot_action(self, action_id: str, status: str, error: Optional[str] = None) -> None:
        await self.db.try_run(lambda c: c.table("bot_action_queue").update({"status": status, "error": error, "processed_at": _now()}).eq("id", action_id).execute())
    ...
    async def save_voice_stats(self, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("voice_stats").upsert(payload, on_conflict="guild_id,user_id").execute())
    ...
    async def touch_command_cooldown(self, guild_id: str, command: str, user_id: str) -> None:
        await self.db.try_run(lambda c: c.table("command_cooldowns").upsert({"guild_id": guild_id, "command": command, "user_id": user_id, "last_used_at": _now()}, on_conflict="guild_id,command,user_id").execute())
    ...
    async def set_command_enabled(self, guild_id: str, command: str, enabled: bool) -> None:
        await self.db.try_run(lambda c: c.table("guild_command_settings").upsert({"guild_id": guild_id, "command": command, "enabled": enabled, "updated_at": _now()}, on_conflict="guild_id,command").execute())
    ...
    async def set_feature_state(self, guild_id: str, key: str, value: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("guild_feature_state").upsert({"guild_id": guild_id, "key": key, "value": value, "updated_at": _now()}, on_conflict="guild_id,key").execute())
    ...
    async def list_command_records(self, guild_id: str, namespace: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    async def delete_command_records(self, guild_id: str, namespace: str, label: Optional[str] = None) -> int:
        existing = await self.list_command_records(guild_id, namespace, limit=100); targets = [r for r in existing if not label or (r.get("label") or "").lower() == label.lower()]
    ...
    async def log_command_usage(self, guild_id: str, command: str, category: str, user_id: str) -> None:
        await self.db.try_run(lambda c: c.table("command_usage").insert({"guild_id": guild_id, "command": command, "category": category, "user_id": user_id}).execute())

    async def command_usage_count(self, guild_id: str, command: str) -> int:
        rows = await self.db.try_run(lambda c: c.table("command_usage").select("id").eq("guild_id", guild_id).eq("command", command).limit(1000).execute()); return len(getattr(rows, "data", None) or [])

    async def command_uses_since(self, guild_id: str, command: str, user_id: str, since_iso: str) -> int:
        rows = await self.db.try_run(lambda c: c.table("command_usage").select("id").eq("guild_id", guild_id).eq("command", command).eq("user_id", user_id).gte("created_at", since_iso).limit(1000).execute()); return len(getattr(rows, "data", None) or [])
    ...
    async def log_error(self, *, source: str, error_type: str, message: str, guild_id: Optional[str] = None, command: Optional[str] = None, traceback_text: Optional[str] = None, user_id: Optional[str] = None, channel_id: Optional[str] = None, cause: Optional[str] = None, location: Optional[str] = None, context: Optional[str] = None) -> None:
        payload = {"guild_id": guild_id, "source": source, "command": command, "error_type": error_type, "message": message[:2000], "traceback": (traceback_text or "")[:8000] or None, "user_id": user_id, "channel_id": channel_id}
        if cause is not None: payload["cause"] = cause[:2000]
        if location is not None: payload["location"] = location[:500]
        if context is not None: payload["context"] = context[:2000]
        await self.db.try_run(lambda c: c.table("bot_error_logs").insert(payload).execute())

    async def bump_message_activity(self, guild_id: str, user_id: str, channel_id: str, day_iso: str) -> None:
        rows = await self.db.try_run(lambda c: c.table("message_activity").select("count").eq("guild_id", guild_id).eq("user_id", user_id).eq("channel_id", channel_id).eq("day", day_iso).limit(1).execute()); data = getattr(rows, "data", None) or []; current = int(data[0]["count"]) if data else 0
        await self.db.try_run(lambda c: c.table("message_activity").upsert({"guild_id": guild_id, "user_id": user_id, "channel_id": channel_id, "day": day_iso, "count": current + 1, "updated_at": _now()}, on_conflict="guild_id,user_id,channel_id,day").execute())

    async def bump_voice_activity(self, guild_id: str, user_id: str, channel_id: str, day_iso: str, seconds: int) -> None:
        if seconds <= 0: return
        rows = await self.db.try_run(lambda c: c.table("voice_activity").select("seconds").eq("guild_id", guild_id).eq("user_id", user_id).eq("channel_id", channel_id).eq("day", day_iso).limit(1).execute()); data = getattr(rows, "data", None) or []; current = int(data[0]["seconds"]) if data else 0
        await self.db.try_run(lambda c: c.table("voice_activity").upsert({"guild_id": guild_id, "user_id": user_id, "channel_id": channel_id, "day": day_iso, "seconds": current + seconds, "updated_at": _now()}, on_conflict="guild_id,user_id,channel_id,day").execute())

    async def record_member_count(self, guild_id: str, day_iso: str, count: int) -> None:
        await self.db.try_run(lambda c: c.table("member_count_daily").upsert({"guild_id": guild_id, "day": day_iso, "member_count": count, "updated_at": _now()}, on_conflict="guild_id,day").execute())
    ...
    async def message_leaderboard(self, guild_id: str, since_day_iso: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for row in getattr(rows, "data", None) or []: totals[row["user_id"]] = totals.get(row["user_id"], 0) + int(row.get("count", 0) or 0)
        return [{"user_id": uid, "messages": total} for uid, total in sorted(totals.items(), key=lambda kv: kv[1], reverse=True)[:limit]]
    ...
    async def channel_leaderboard(self, guild_id: str, since_day_iso: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for row in getattr(rows, "data", None) or []: totals[row["channel_id"]] = totals.get(row["channel_id"], 0) + int(row.get("count", 0) or 0)
        return [{"channel_id": cid, "messages": total} for cid, total in sorted(totals.items(), key=lambda kv: kv[1], reverse=True)[:limit]]
    ...
    async def voice_user_leaderboard(self, guild_id: str, since_day_iso: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for row in getattr(rows, "data", None) or []: totals[row["user_id"]] = totals.get(row["user_id"], 0) + int(row.get("seconds", 0) or 0)
        return [{"user_id": uid, "voice_seconds": total} for uid, total in sorted(totals.items(), key=lambda kv: kv[1], reverse=True)[:limit]]

    async def message_series(self, guild_id: str, since_day_iso: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("message_activity").select("day, count").eq("guild_id", guild_id).gte("day", since_day_iso).limit(20000).execute()); by_day: dict[str, int] = {}
        for row in getattr(rows, "data", None) or []: by_day[str(row["day"])] = by_day.get(str(row["day"]), 0) + int(row.get("count", 0) or 0)
    ...
    async def voice_series(self, guild_id: str, since_day_iso: str) -> list[dict[str, Any]]:
        rows = await self.db.try_run(lambda c: c.table("voice_activity").select("day, seconds").eq("guild_id", guild_id).gte("day", since_day_iso).limit(20000).execute()); by_day: dict[str, int] = {}
        for row in getattr(rows, "data", None) or []: by_day[str(row["day"])] = by_day.get(str(row["day"]), 0) + int(row.get("seconds", 0) or 0)
    ...
    async def increment_server_counter(self, guild_id: str, column: str, amount: int = 1) -> None:
        if amount == 0: return
        rows = await self.db.try_run(lambda c: c.table("servers").select(column).eq("guild_id", guild_id).limit(1).execute()); data = getattr(rows, "data", None) or []; current = int(data[0].get(column) or 0) if data else 0
        await self.db.try_run(lambda c: c.table("servers").update({column: current + amount}).eq("guild_id", guild_id).execute())
    ...
    async def touch_user_activity(self, guild_id: str, user_id: str, username: str, fields: dict[str, Any]) -> None:
        payload = {"guild_id": guild_id, "user_id": user_id, "username": username, "last_seen_at": _now(), "updated_at": _now(), **fields}; await self.db.try_run(lambda c: c.table("user_activity").upsert(payload, on_conflict="guild_id,user_id").execute())

    async def touch_user_message(self, guild_id: str, user_id: str, username: str, channel_id: str, content: str) -> None:
        await self.touch_user_activity(guild_id, user_id, username, {"last_message_at": _now(), "last_message_content": content[:500], "last_message_channel_id": channel_id})

    async def touch_user_voice(self, guild_id: str, user_id: str, username: str, *, joined: bool) -> None:
        await self.touch_user_activity(guild_id, user_id, username, {"last_voice_join_at" if joined else "last_voice_leave_at": _now()})

    async def touch_user_presence(self, guild_id: str, user_id: str, username: str, status: str) -> None:
        await self.touch_user_activity(guild_id, user_id, username, {"last_online_status": status})

    async def touch_user_command(self, guild_id: str, user_id: str, username: str, command_name: str) -> None:
        current = await self.get_user_activity(guild_id, user_id); count = int(current.get("command_count") or 0) + 1
        await self.touch_user_activity(guild_id, user_id, username, {"last_command_at": _now(), "last_command_name": command_name, "command_count": count})
    ...
    async def save_roll_call_settings(self, guild_id: str, fields: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("roll_call_settings").upsert({"guild_id": guild_id, "updated_at": _now(), **fields}, on_conflict="guild_id").execute())
    ...
    async def set_roll_call_message(self, roll_call_id: str, message_id: str) -> None:
        await self.db.try_run(lambda c: c.table("roll_calls").update({"message_id": message_id}).eq("id", roll_call_id).execute())

    async def close_roll_call(self, roll_call_id: str) -> None:
        await self.db.try_run(lambda c: c.table("roll_calls").update({"status": "closed", "updated_at": _now()}).eq("id", roll_call_id).execute())
    ...
    async def save_streak(self, payload: dict[str, Any]) -> None:
        await self.db.try_run(lambda c: c.table("roll_call_streaks").upsert(payload, on_conflict="guild_id,user_id").execute())
