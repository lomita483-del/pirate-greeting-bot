...
def _build_embed(data: dict) -> discord.Embed | None:
    ...
    if data.get("author_name"):
        embed.set_author(name=data["author_name"], url=data.get("author_url") or None, icon_url=data.get("author_icon_url") or None)
    ...
    for field in (data.get("fields") or [])[:25]:
        embed.add_field(name=(field.get("name") or "\u200b")[:256], value=(field.get("value") or "\u200b")[:1024], inline=bool(field.get("inline")))
...
class SendCommands(commands.Cog):
    ...
    async def send(self, interaction: discord.Interaction, channel: discord.TextChannel, message_content: str, message_title: str | None = None, mention_role: discord.Role | None = None, embed: bool = False, embed_color: app_commands.Choice[str] | None = None) -> None:
        ...
        try:
            await channel.send(content=content, embed=rich_embed, allowed_mentions=discord.AllowedMentions(everyone=False, roles=bool(mention_role), users=True))
    ...
    async def _process_rollcall_start(self, action: dict) -> None:
        ...
        if not roll_call or roll_call.get("status") != "open":
            raise ActionRefused("Roll call no longer exists or is already closed.")
        guild = self.bot.get_guild(int(action["guild_id"]))
        if guild is None:
            raise ActionRefused("AHOY is not in that server.")
        channel = guild.get_channel(int(roll_call["channel_id"]))
        if not isinstance(channel, discord.TextChannel):
            raise ActionRefused("Roll call channel no longer exists.")
        roles = [guild.get_role(int(x)) for x in roll_call.get("target_role_ids") or []]
        roles = [r for r in roles if r is not None]
        view = RollCallView(self.bot, str(roll_call["id"]))
    ...
    async def _process_rollcall_close(self, action: dict) -> None:
        ...
        if not isinstance(cog, RollCall):
            raise ActionRefused("Roll Call service is not ready.")
        await cog._close_roll_call(guild, roll_call)
        await repo.finish_bot_action(action["id"], "done")
